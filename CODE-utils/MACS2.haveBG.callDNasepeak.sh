...
if [ $# -lt 7 ];then
	    echo "Need 7 parameters! <InputDir> <sample> <Species> <Clean.bw> <Clean.np> <OutDir> <Control-ffor call peaks>"
		    exit
fi
...
####--------run macs2
mkdir -p ${Out}/${In}
macs2 callpeak -t ${DIR}/${In}.bam -q 0.01 --extsize 147 --nomodel -g ${Species} -B -f BAM --verbose 0 -n ${Out}/${In}/${In}.macs2.ext147.0.01 -c control
